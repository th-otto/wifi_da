...
/*----------------------------------------------------------------------------------------*/ 
static _WORD __CDECL draw_arrow(PARMBLK *parmblock)
{
	BITBLK *image;
	MFDB src;
	MFDB des;
	...
	vs_clip(aes_handle, 1, clip); /* Zeichenoperationen auf gegebenen Bereich beschraenken */
	...
	vrt_cpyfm(aes_handle, MD_REPLACE, xy, &src, &des, image_colors);
...
/*----------------------------------------------------------------------------------------*/ 
static void __CDECL strs_init(struct POPUP_INIT_args args)
{
	OBJECT *obj;
	PSTRS *popup_par;
	short max_strs;
...
/*----------------------------------------------------------------------------------------*/ 
_WORD do_popup(GRECT *button_rect, char **strs, _WORD no_strs, _WORD spaces, _WORD slct)
{
	PSTRS popup_par;
	OBJECT *tree;
	_WORD max_width;
	...
	if (tree) /* Speicher vorhanden? */
	{
		...
		max_width *= gl_wchar; /* Breite in Pixeln */
		...
		tree->ob_height = max_strs * gl_hchar; /* Hoehe der Box */
		...
		tree->ob_y -= (slct - scroll_pos) * gl_hchar; /* Popup nach oben verschieben */
...
#pragma warn +stv
		...
		if (no_strs > MAX_STRS)
		{
			if (aes_flags & GAI_SCROLLPOPUP)
				...
				slct = do_form_popup(tree, 0, 0, 1, max_strs, no_strs, (void *) &popup_par, &scroll_pos);
...
static void pop_position(OBJECT *tree, GRECT *work)
{
	...
	if ((y + work->g_h) > (screen.g_y + screen.g_h))
		y = screen.g_y + screen.g_h - work->g_h;
	if (y < screen.g_y)
		y = screen.g_y;

	if ((x + work->g_w) > (screen.g_x + screen.g_w))
		x = screen.g_x + screen.g_w - work->g_w;
	if (x < screen.g_x)
		x = screen.g_x;
...
static bool pop_arrow(OBJECT *tree, _WORD obj, _WORD *offset, _WORD no_strs, void *param)
{
	...
	_WORD *offset)
{
	...
	graf_mkstate(&mox, &moy, &mbutton, &dummy);
	exit_state = mbutton ^ 1;
	...
	do
	{
		...
		if ((events & MU_BUTTON) && obj > 0 && (obj == firstscrlob || obj == lastscrlob))
		{
